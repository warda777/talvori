...
class LocalLearningTestScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Intensiv lernen'),
      ),
  ...
  Future<void> _startOrResume(WidgetRef ref) async {
    ...
    await ref.read(localLearningControllerProvider.notifier).startOrResume(
      categoryId: categoryId,
      mode: mode,
      trainingArea: trainingArea,
      now: now,
  ...
  Future<void> _submitCorrect(WidgetRef ref) async {
    ...
    await ref.read(localLearningControllerProvider.notifier).submitCorrect(
      now: now,
    );
  }
  ...
  Future<void> _submitWrong(WidgetRef ref) async {
    ...
    await ref.read(localLearningControllerProvider.notifier).submitWrong(
      now: now,
    );
  }
  ...
  Future<void> _completeIfFinished(WidgetRef ref) async {
    ...
    await ref.read(localLearningControllerProvider.notifier).completeIfFinished(
      now: now,
    );
  }
