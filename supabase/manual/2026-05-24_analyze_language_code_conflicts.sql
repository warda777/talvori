-- Manual conflict analysis for EN/DE -> en/de language-code normalization.
--
-- Purpose:
--   Analyze the 25 EN/DE candidates before any further normalization attempt.
--
-- Safety notes:
--   - SELECT-only script.
--   - Run manually in the Supabase SQL Editor.
--   - Does not update, delete, merge, or move any data.
--   - Does not touch categories, word_progress, user_word_srs, user_words,
--     pass_count, stage, is_mastered, or next_due_at.
...
-- 2. Conflicts against existing en/de rows with the same text.
WITH candidates(id) AS (
  VALUES
    ('0003d2c4-9558-42b6-bf79-aa099e3f4701'::uuid),
    ('42cba640-454e-447b-b0e0-e5a44f8968c7'::uuid),
...
SELECT
  ...
  candidate.text AS candidate_text,
  candidate.translation AS candidate_translation,
  conflict.id AS conflicting_id,
  conflict.text AS conflicting_text,
  conflict.translation AS conflicting_translation,
  'same_text_lang' AS conflict_type
FROM candidate_words candidate
JOIN public.words conflict
  ON conflict.text = candidate.text
 AND conflict.from_lang = 'en'
 AND conflict.to_lang = 'de'
 AND conflict.id <> candidate.id
ORDER BY candidate.text, conflict.id;

-- 3. Exact duplicates: same text, same translation, en/de.
WITH candidates(id) AS (
  VALUES
    ('0003d2c4-9558-42b6-bf79-aa099e3f4701'::uuid),
    ('42cba640-454e-447b-b0e0-e5a44f8968c7'::uuid),
...
SELECT
  ...
  conflict.text AS conflicting_text,
  conflict.translation AS conflicting_translation,
  'same_text_translation_lang' AS conflict_type
FROM candidate_words candidate
JOIN public.words conflict
  ON conflict.text = candidate.text
 AND conflict.translation = candidate.translation
 AND conflict.from_lang = 'en'
 AND conflict.to_lang = 'de'
 AND conflict.id <> candidate.id
ORDER BY candidate.text, conflict.id;

-- 4. Candidates with no same-text en/de conflict.
WITH candidates(id) AS (
  VALUES
    ('0003d2c4-9558-42b6-bf79-aa099e3f4701'::uuid),
    ('42cba640-454e-447b-b0e0-e5a44f8968c7'::uuid),
...
SELECT
  ...
  candidate.text AS candidate_text,
  candidate.translation AS candidate_translation,
  'en' AS target_from_lang,
  'de' AS target_to_lang
FROM candidate_words candidate
WHERE NOT EXISTS (
  SELECT 1
