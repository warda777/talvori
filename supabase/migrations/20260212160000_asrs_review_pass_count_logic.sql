-- A-SRS: Review-Logik mit pass_count statt streak
-- Regel:
--   S0 -> S1 sofort (nicht hier, sondern fn_a_srs_s0_correct)
--   S1 -> S2 nach 2x richtig
--   S2 -> S3 nach 2x richtig
--   S3 -> S4 nach 2x richtig
--   S4 -> S5 nach 3x richtig
--   S5 -> mastered nach 3x richtig
-- Falsch: pass_count=0, stage <= 1 -> 1, sonst stage-1
--
-- Anzeige: A5 = stage=5 AND is_mastered=false, mastered = is_mastered=true
