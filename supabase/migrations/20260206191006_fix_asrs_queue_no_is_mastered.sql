-- Fix: A‑SRS Queue darf keine Spalten referenzieren, die in user_word_srs nicht existieren.
--
-- Symptom (aus Flutter-Log):
-- PostgrestException: column uws.is_mastered does not exist
--
-- Ursache:
-- user_word_srs hat im aktuellen Schema kein is_mastered (das Feld existiert in word_progress,
-- aber unsere adaptive Queue arbeitet auf user_word_srs).
--
-- Lösung:
-- - Entferne den Filter auf uws.is_mastered in fn_user_learn_queue_adaptive_impl
-- - Mastering/Completed muss (wenn gewünscht) über ein anderes Feld/andere Tabelle modelliert werden.
