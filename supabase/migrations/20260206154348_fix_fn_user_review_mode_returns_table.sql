-- Fix: fn_user_review_mode sollte eine Tabelle zurückgeben, nicht void
-- Der Dart-Code erwartet srs_stage und next_due_at
-- PostgreSQL erlaubt keine Änderung des Rückgabetyps, daher müssen wir die Funktion zuerst löschen
...
BEGIN
  ...
  SELECT * FROM public.fn_user_review_mode_text(
    ...
    p_category,
    p_word,
    p_mode::text,  -- Cast srs_mode enum to text
    p_result
  );
END;
$function$
...
-- Fix: fn_user_review_mode_text muss mode mit p_mode::srs_mode vergleichen, nicht direkt mit text
