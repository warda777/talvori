-- A-SRS: S5 + 3 richtig → is_mastered = true
-- Anzeige: Weiß (erstes Mal), Hellblau (1× richtig), Grün (2× richtig), beim 3. → Mastered
-- Wort kommt nach ca. 8 Karten wieder (Final Round insert bei index+8)
...
begin
  ...
  -- Guard: Bereits gemasterte Wörter nicht erneut reviewen
  ...
  if p_result then
    if v_stage = 1 then
      ...
      -- S5: Weiß→Hellblau→Grün→Mastered (3× richtig)
      v_new_pass_count := coalesce(v_pass_count, 0) + 1;
      v_new_stage := 5;
      if v_new_pass_count >= 3 then
