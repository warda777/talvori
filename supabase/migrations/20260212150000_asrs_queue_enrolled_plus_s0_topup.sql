-- A-SRS Queue: enrolled (S1-S5) + S0-Top-up kombinieren
-- Vorher: S0 nur wenn Queue leer (if v_cnt = 0)
-- Nachher: Zuerst fällige enrolled Karten, dann mit S0 auffüllen bis Deck voll
...
begin
  ...
  -- 1) Fällige enrolled Karten (S1-S5) zuerst
  ...
  v_remaining := v_take - v_cnt;

  -- 2) Mit S0 auffüllen, bis Deck voll (nicht nur wenn leer!)
  if v_remaining > 0 then
    ...
    where uws.user_id = v_user
      ...
      and uws.word_id not in (select out_word_id from _asrs_queue_tmp)
