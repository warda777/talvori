-- v_words_user_srs: pass_count für Bounce-Logik (A-SRS) hinzufügen
-- Der Client braucht pass_count, um zu wissen, ob Bounce bei "bleibt in Stage" oder "Stage up" erfolgt.
-- DROP nötig: CREATE OR REPLACE erlaubt keine Spaltenänderung (42P16).

DROP VIEW IF EXISTS "public"."v_words_user_srs";

CREATE VIEW "public"."v_words_user_srs" AS
WITH base AS (
  SELECT
    w.id AS word_id,
    w.text,
...
SELECT
  ...
  uws.ef,
  uws.streak,
  COALESCE(uws.pass_count, 0) AS pass_count,
  uws.lapses
FROM base b
LEFT JOIN u
