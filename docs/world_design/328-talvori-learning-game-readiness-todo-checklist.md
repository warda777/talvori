...
M16-DN operationalisiert fuer kuenftige Slices:

- `docs/world_design/397-uferwald-json-yaml-planning-format-review.md`
  bestaetigt M16-DM als ausreichend und verneint M16-DM-FIX.
- M16-DN entscheidet: M16-DO Uferwald YAML Planning Skeleton Gate darf
  vorbereitet werden.
- Eine echte `.yaml`-Datei darf in M16-DO nur entstehen, wenn der Folgeprompt
  Pfad, Dateiname, Maximalstatus, Statusschutz und Datei-Check ausdruecklich
  oeffnet.
- `.json` und `.yml` bleiben ohne separate explizite Folgefreigabe blockiert.
...
M16-DQ operationalisiert fuer kuenftige Slices:

- `docs/world_design/400-uferwald-measurement-value-gate-preparation.md`
  entscheidet `anchor_review_values` als erste spaeter oeffnungsfaehige
  Wertfamilie.
- `anchor_review_values` duerfen im ersten Value-Pfad nur Rollen-, Modus-,
  Status- und QA-Werte vorbereiten; Koordinaten, `normalized_x`,
  `normalized_y`, Polygone, Path-Centerlines, Path-Nodes, Path-Edges,
  No-Walk-/No-Build-Unionen und Pixelableitung bleiben verboten.
- Path-, Water-, No-Walk-, No-Build-, Sort-Band- und Buildable-Zone-
  Wertfamilien bleiben fuer den ersten Value-Pfad blockiert, weil sie zu
  schnell Runtime-, Geometrie-, Collision-, Renderer- oder Slot-Logik
  suggerieren.
- Der naechste sinnvolle Folge-Slice ist M16-DR Uferwald Anchor Review Values
  Gate. M16-DR soll weiterhin Markdown-only bleiben und keine YAML-Aenderung
  erzeugen.
...
M16-DR operationalisiert fuer kuenftige Slices:

- `docs/world_design/401-uferwald-anchor-review-values-gate.md` definiert den
  exakten Markdown-Feldvertrag fuer `anchor_review_values`.
- Erlaubt sind nur Vertragselemente fuer `value_family`, `value_status`,
  `anchor_id`, `anchor_review_role`, `mode_relevance`,
  `measurement_requirement`, Statusschutz, QA und `blocked_uses`.
- Bekannte Anchor-IDs duerfen spaeter nur als reine IDs ohne Koordinaten
  referenziert werden.
- Rollen wie `landmark`, `hub_reference`, `build_reference`,
  `object_focus_reference`, `river_reference`, `grove_reference`,
  `reserve_reference` und `path_orientation_reference` bleiben Review-Rollen,
  keine Runtime-Anker.
- YAML-Aenderungen, echte Werte, Koordinaten, Pixelwerte, Polygone,
  Path-Centerlines, Path-Nodes, Path-Edges, Build-Zonen-Polygone, No-Walk-/
  No-Build-Unionen, Runtime-Daten, Assets, App-Integration und Code bleiben
  fuer M16-DR blockiert.
- Der naechste sinnvolle Folge-Slice ist M16-DS Uferwald Anchor Review Values
...
Italien-Kontur Source-of-Truth Gate erledigt:

- M16T-WORLD-009

Das Source-of-Truth-Gate operationalisiert fuer kuenftige Slices:

- `docs/world_design/405-italy-shape-source-of-truth-gate.md` empfiehlt
  Natural Earth `Admin 0 - Countries` als bevorzugte offene Vektorquelle fuer
  die echte Italien-Aussenkontur.
- OpenStreetMap-basierte Grenzen bleiben nur Kandidat mit separater Lizenz-,
  Attribution- und Share-Alike-Pruefung.
- Festland Italien ist grundsaetzlich enthalten; Sizilien und Sardinien sind
  zu pruefen und voraussichtlich enthalten; kleine Nebeninseln bleiben nur bei
  Gameplay-Relevanz.
- Die Kontur darf fuer Mobile-Lesbarkeit vereinfacht werden, muss aber als
  Italien erkennbar bleiben und darf nicht wie Unterrichtsatlas oder GIS-Tool
  wirken.
- Dieses Gate erzeugt keine Konturdatei, keine SVG/PNG, keine Bilder, keine
  Assets, keine Runtime-Daten, keine Koordinaten, keine YAML-/JSON-Dateien und
  keinen Code.
...
Italien-Makro-Blockout erledigt:

- M16T-WORLD-011

Das Blockout Gate operationalisiert fuer kuenftige Slices:

- `docs/world_design/407-italy-macro-blockout-paths-water-buildareas.md`
  dokumentiert den ersten sichtbaren Makro-Blockout fuer Italien.
- `docs/world_design/previews/italy_macro_blockout_paths_water_buildareas/`
  enthaelt SVG, PNG und Metadata fuer Wege, Wasser/Kueste, Uebergaenge,
  Bauflaechen und grobe No-Walk-/No-Build-Bereiche.
- Der Blockout nutzt die 406-Arbeitskontur als wiederverwendete Grundlage und
  fuegt lokale Dokumentations-Overlays hinzu.
- Statusschutz: `documentation_only`, `not_asset`, `not_runtime_data` und
  `not_engine_ready`.
- Der naechste empfohlene Slice ist: Italien technische Layer/Masks fuer
  Makro-Blockout planen.

Italien-Stadtanker fuer Makro-Blockout erledigt:

- M16T-WORLD-012

Das Stadtanker-Gate operationalisiert fuer kuenftige Slices:

- `docs/world_design/408-italy-city-anchor-plan.md` legt echte italienische
  Stadtanker fuer die Ueberarbeitung des Makro-Blockouts fest.
- Sofort wichtig: Rom, Florenz, Venedig, Mailand, Neapel und Bologna.
- Reserve: Pisa, Verona, Genua, Bari, Palermo, Catania und Cagliari.
- Natural Earth Populated Places ist der bevorzugte spaetere offene
  Stadtpunkt-Kandidat; OSM und ISTAT bleiben nur Kandidaten mit eigenem
  Lizenz-/Attributionsgate.
...
Italien spielbare Stadtbereiche aus Stadtgrundformen erledigt:

- M16T-WORLD-015

Das Playable-City-Area-Gate operationalisiert fuer kuenftige Slices:

- `docs/world_design/411-italy-playable-city-areas-from-footprints.md`
  uebersetzt die 13 ISTAT-Comuni-Footprints in abstrahierte
  `playable_city_area`-Kandidaten.
- Jede Stadt erhaelt im Dokumentationsvisual grob Stadtkern, Reserveflaeche,
  Rand/No-Build und moeglichen Start-Bauplatz.
- Kernstaedte werden staerker ausgearbeitet: Milano, Venezia, Bologna,
  Firenze, Roma und Napoli.
- Reserve-Staedte bleiben dezenter: Genova, Pisa, Verona, Bari, Palermo,
  Catania und Cagliari.
- Die echten ISTAT-Grenzen bleiben Source-Footprints. Sie sind keine
  Runtime-Polygone, keine finalen Koordinaten, keine Build-Zones, keine
...
World-Travel Zoom Flow als Spielnavigation erledigt:

- M16T-WORLD-017

Das Gameflow-Gate operationalisiert fuer kuenftige Slices:

- `docs/world_design/413-world-travel-zoom-navigation-gameflow-gate.md`
  legt fest, dass Talvori nicht die 411-Dokumentationskarte als iPhone-Screen
  nutzt.
- Der Ziel-Flow ist: Globus/Welt -> Europa/Weltregion -> Land auswaehlen ->
  Italien-Landansicht -> Stadtanker auswaehlen -> Firenze-Stadtansicht ->
  Bauflaechen, Wege, Lernorte und Gebaeude.
- Benchmark-Muster aus Pokémon GO, Genshin Impact/HoYoLAB, Clash of Clans und
  Everdale werden nur als Spielgefuehl-/Navigationsmuster genutzt; keine
  fremden Screenshots, Assets, Kartenbilder, Code oder Daten werden
  uebernommen.
- Italien ist erstes aktives Land; andere Laender sollen sichtbar, aber locked
  oder ausgegraut bleiben.
